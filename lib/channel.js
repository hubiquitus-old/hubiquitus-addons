...
function Channel(id, channelImpl) {
  ...
  function subscribe(from, content, reply) {
    onSubscribe(from, content ? (content.mode || 'full') : 'full', function (err) {
      reply(err);
    });
  }

  function unsubscribe(from, content, reply) {
    onUnsubscribe(from, function (err) {
      reply(err);
    });
  }
}
...
function createInMemoryChannel() {
  ...
  function onSubscribe(subscriber, mode, cb) {
