...
function Gateway(session, login) {
  ...
  _this.sock.on('connection', function (socket) {
    logger.debug('gateway connection from ' + socket.remoteAddress);
  ...
  function processLogin(socket, data) {
    ...
    login(socket, data, function (err, identity) {
      if (!err) {
        ...
        logger.debug('gateway login success from ' + socket.remoteAddress + '; identifier : ' + identity);
  ...
  function processMessage(socket, message) {
    if (tv4.validate(message, schemas.message)) {
      ...
      hubiquitus.send(message.from || socket.identity, message.to, message.payload.content, function (err, from, content) {
  ...
  function processResponse(response) {
    logger.debug('gateway processing response', response);
  ...
  function processQueue(socket) {
    ...
    logger.debug('gateway processing ' + socket.identity + ' queue (' + socket.queue.length + ' elements)');
