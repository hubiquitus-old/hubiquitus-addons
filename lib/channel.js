...
exports.createChannel = function (id, channelImpl) {
  if (channelImpl) {
    if (!_.isFunction(channelImpl.getSubscribers) ||
  ...
  return new Channel(id, channelImpl || createInMemoryChannel());
};
...
function Channel(id, channelImpl) {
  ...
  var getSubscribers = channelImpl.getSubscribers;
  ...
  function publish(from, content) {
    ...
    var ctx = this;
    _.forEach(getSubscribers(), function (subscriber) {
